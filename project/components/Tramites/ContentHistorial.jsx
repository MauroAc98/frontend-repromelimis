...
export default function ContentHistorial({ tramiteData }) {
  ...
  const habilitarCargaDatosFaltantes = async () => {
    try {
      ...
      const { data } = await api.post(
        `/tramites/habilitar-carga-datos-faltantes/${tramiteData.id}`
      );
      showSuccessMsg(data.message);
      router.push("/tramites/productores");
    } catch (error) {
  ...
  return (
    <>
      ...
      />
      {tramiteData.estado_actual == "DATOS_INSUFICIENTES" && (
